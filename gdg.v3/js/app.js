...
const App = (() => {
  ...
  async function boot() {
    ...
    //    This fires immediately with current data AND on every future change
  ...
  // ── Offline fallback: seed local state if Firebase not configured ──
  function seedLocalFallback() {
    const dummyReports = [
      ...
      { id: 1006, severity: 1, status: 'resolved', description: 'Plastic bags around Community Park. Cleaned up by volunteer group.', lat: 12.9856, lng: 77.5963, reporter: 'Vikram Singh', timestamp: Date.now() - 86400000, claimedBy: 'Anita Desai', timeline: [{ action: 'Reported', time: Date.now() - 86400000, actor: 'Vikram Singh' }, { action: 'Area cleaned up! 🎉', time: Date.now() - 43200000, actor: 'Anita Desai' }], photoUrl: null },
      { id: 1007, severity: 3, status: 'pending', description: 'Sewage overflow mixing with garbage. Health hazard near residential area.', lat: 12.9550, lng: 77.6010, reporter: 'Sunita Rao', timestamp: Date.now() - 1800000, timeline: [{ action: 'Reported', time: Date.now() - 1800000, actor: 'Sunita Rao' }], photoUrl: null, claimedBy: null },
    ];
    ...
    // Manually trigger the render since startRealtimeSync won't call back
    setTimeout(() => {
  ...
  function bindEvents() {
    ...
    document.getElementById('btn-submit-report')?.addEventListener('click', async () => {
      ...
      // ── Compress photo → base64 (free, no Storage needed) ──
      ...
      await CLEANMAP.createReport({
        ...
        photoUrl,    // ← permanent Firebase Storage URL (null if no photo)
      });

      UI.closeModal('modal-report');
      UI.toast('Report Submitted! 📍', 'Visible on all connected devices instantly.', 'success');
      UI.switchScreen('map');
  ...
  async function claimSpot(id) {
    ...
    const userName = window._reporterName || 'Volunteer';
    const success = await CLEANMAP.claimReport(id, userName);
    if (success) {
      UI.toast('Spot Claimed! 🧹', 'Status updated live on all devices.', 'success');
    } else {
      UI.toast('Already Claimed', 'This spot is already in progress or cleaned.', 'warning');
    }
  }

  async function resolveSpot(id, afterPhotoUrl = null) {
    const status = await CLEANMAP.resolveReport(id, afterPhotoUrl);
    if (status === 'resolved') {
      UI.toast('Area Cleaned! 🎉', 'Marked as resolved on all devices.', 'success');
    } else if (status === 'pending_proof') {
      UI.toast('Pending Proof 📷', 'Work done, but needs an after photo to be verified.', 'warning');
    } else {
      UI.toast('Cannot Resolve', 'Spot must be "In Progress" first.', 'warning');
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // Ask for a name on first visit (stored in localStorage)
    const savedName = localStorage.getItem('cleanmap_username');
    if (savedName) {
      window._reporterName = savedName;
    } else {
      // Will be set when user submits first report
