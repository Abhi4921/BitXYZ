...
const CLEANMAP = (() => {
  ...
  const STATUSES = {
    ...
    pending_proof: { label: 'Pending Proof', icon: '📷', badgeClass: 'badge-pending-proof' },
  ...
  async function createReport({ severity, description, lat, lng, photoUrl, reporter }) {
    ...
    emit('realtimeUpdate', state.reports);
    return report;
  }

  async function claimReport(id, userName = 'Volunteer') {
    const r = state.reports.find(r => r.id === id);
    if (!r || r.status !== 'pending') return false;

    const newTimeline = [
      ...r.timeline,
      { action: 'Claimed for cleanup', time: Date.now(), actor: userName }
    ];
    ...
    addActivity(`🧹 Spot #${id} claimed by ${userName}!`, '🧹');
    emit('realtimeUpdate', state.reports);
    return true;
  }

  async function resolveReport(id, afterPhotoUrl = null) {
    const r = state.reports.find(r => r.id === id);
    if (!r || (r.status !== 'progress' && r.status !== 'pending_proof')) return false;

    const actor = r.claimedBy || 'Volunteer';
    const status = afterPhotoUrl ? 'resolved' : 'pending_proof';

    let actionText = '';
    if (afterPhotoUrl && r.status === 'pending_proof') {
      actionText = 'Proof uploaded. Spot verified! 🎉';
    } else if (afterPhotoUrl && r.status === 'progress') {
      actionText = 'Area cleaned up with proof! 🎉';
    } else {
      actionText = 'Work done, pending photo proof 📷';
    }

    const newTimeline = [...r.timeline];
    if (r.status !== status || afterPhotoUrl) {
      newTimeline.push({ action: actionText, time: Date.now(), actor });
    }

    const updates = { status, timeline: newTimeline };
    if (afterPhotoUrl) updates.afterPhotoUrl = afterPhotoUrl;

    Object.assign(r, updates);

    await FirebaseDB.updateReport(id, updates);

    if (status === 'resolved') {
      addActivity(`✅ Spot #${id} verified clean!`, '✅');
    } else {
      addActivity(`📷 Spot #${id} needs photo proof.`, '📷');
    }
  ...
  function getStats() {
    ...
    return {
      ...
      resolved: all.filter(r => r.status === 'resolved' || r.status === 'pending_proof').length,
      low: all.filter(r => r.severity === 1 && r.status !== 'resolved' && r.status !== 'pending_proof').length,
      med: all.filter(r => r.severity === 2 && r.status !== 'resolved' && r.status !== 'pending_proof').length,
      high: all.filter(r => r.severity === 3 && r.status !== 'resolved' && r.status !== 'pending_proof').length,
    };
  }
  ...
  function getLeaderboard() {
    ...
    state.reports.forEach(r => {
      // Score counts if fully resolved or pending_proof (work done)
      if ((r.status === 'resolved' || r.status === 'pending_proof') && r.claimedBy) {
        const u = r.claimedBy;
        if (!scores[u]) scores[u] = { name: u, score: 0, cleanups: 0 };
        scores[u].score += POINTS[r.severity] || 0;
        scores[u].cleanups += 1;
      }
    });
  ...
  function getScoreForUser(username) {
    if (!username) return 0;
    const item = getLeaderboard().find(x => x.name === username);
