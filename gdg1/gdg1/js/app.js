...
const App = (() => {
  ...
  function bindEvents() {
    ...
    document.getElementById('btn-submit-report')?.addEventListener('click', async () => {
      ...
      const photoUrl = photoInput?.files?.length > 0
        ? URL.createObjectURL(photoInput.files[0]) : null;
      ...
      UI.toast('Report Submitted! 📍', 'It will appear on all connected devices instantly.', 'success');
      UI.switchScreen('map');
